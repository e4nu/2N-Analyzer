...
int main() {
    ...
    for (int i = 0; i < AnalyzeFilePath_v.size(); i++) {
        ...
        DataAnalyser Analysis(FileType, AnalyzeFilePath0, AnalyzeFileSample0, AnalyzeFile0);
        std::string AnalyserMode = Analysis.ConfigureAnalyserMode(FileType);

        cout << "\033[33mAnalyser mode:\033[0m\t'" << AnalyserMode << "'\n";

        cout << "\033[33m\n---------------------------------------------------------------------------\n\n\033[0m";
        cout << "\033[33mAnalysis mode:\033[0m\t'" << AnalyserMode << "'" << "\n";
