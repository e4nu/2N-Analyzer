...
class FDNeutralVeto {
   public:
    ...
    // -------------------------------------------------------------------------
    // Reformatted Step 5: topology-aware handling of indirect 3-layer ECAL rejection
    // -------------------------------------------------------------------------
    // Motivation (physics):
    ...
    //   - Reformatted Step 5 keeps Steps 1–4 identical and makes that indirect rejection conditional,
    //     based on TOF-derived beta classification of the earliest-layer ECAL hit.
    const bool Apply_ReformattedStep5_TopologyAware = true;

    // Candidate classification thresholds (used only when Apply_ReformattedStep5_TopologyAware==true):
    ...
    const double ReformattedStep5_beta_photon_like_min = 0.95;
    const double ReformattedStep5_beta_neutron_like_max = 0.90;
   ...
   private:
    ...
    enum class ReformattedStep5CandidateType { PhotonLike, NeutronLike, Ambiguous };

    // Option C candidate classifier.
    // Physics intent:
    ...
    ReformattedStep5CandidateType ClassifyReformattedStep5_Candidate(const NeutralHit& earliestLayerHit) const;

    // Option C decision: whether to apply the indirect "3-layer" rejection (num_of_ECAL_hits_ahead < 2)
    // for the current earliest-layer ECAL survivor.
    bool ApplyReformattedStep5_IndirectThreeLayerRejection(const NeutralHit& earliestLayerHit) const;
