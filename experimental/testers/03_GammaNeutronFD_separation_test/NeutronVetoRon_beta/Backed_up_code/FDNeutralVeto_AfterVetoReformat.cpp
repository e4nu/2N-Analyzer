...
FDNeutralVeto::ReformattedStep5CandidateType FDNeutralVeto::ClassifyReformattedStep5_Candidate(const NeutralHit& earliestLayerHit) const {
    ...
    const auto [tof, beta, gamma, pvec] = Calc_nFD_vars(earliestLayerHit, m_starttime);

    // If beta is not a number (non-physical TOF), treat as ambiguous and let other veto steps dominate.
    if (!std::isfinite(beta)) { return ReformattedStep5CandidateType::Ambiguous; }

    if (beta >= ReformattedStep5_beta_photon_like_min) { return ReformattedStep5CandidateType::PhotonLike; }
    if (beta <= ReformattedStep5_beta_neutron_like_max) { return ReformattedStep5CandidateType::NeutronLike; }

    return ReformattedStep5CandidateType::Ambiguous;
}

bool FDNeutralVeto::ApplyReformattedStep5_IndirectThreeLayerRejection(const NeutralHit& earliestLayerHit) const {
    ...
    if (!Apply_ReformattedStep5_TopologyAware) { return true; }

    const ReformattedStep5CandidateType type = ClassifyReformattedStep5_Candidate(earliestLayerHit);

    if (type == ReformattedStep5CandidateType::PhotonLike) { return true; }
...
struct FDNeutralVeto::DebugHists {
    struct VarDef {
        std::string key;
        std::string title;
        int nb;
    ...
    std::vector<VarDef> hitVars = {
        {"p", "p [GeV]", 50, 0.0, 10.0},
        {"theta", "theta [deg]", 50, 0.0, 60.0},
        {"phi", "phi [deg]", 50, -180.0, 180.0},
        {"time", "time [ns]", 50, -50.0, 150.0},
        {"hittime", "HitTime [ns]", 50, -50.0, 150.0},
        {"vx", "Vx [cm]", 50, -20.0, 20.0},
        {"vy", "Vy [cm]", 50, -20.0, 20.0},
        {"vz", "Vz [cm]", 50, -40.0, 40.0},
    };

    // Step comparison vars (differences + opening angle + sector difference)
    std::vector<VarDef> stepVars = {
        {"dp", "p(test) - p(ref) [GeV]", 50, -5.0, 5.0},
        {"dtheta", "theta(test) - theta(ref) [deg]", 50, -60.0, 60.0},
        {"dphi", "phi(test) - phi(ref) [deg]", 50, -180.0, 180.0},
        {"dtime", "Time(test) - Time(ref) [ns]", 50, -50.0, 50.0},
        {"dhittime", "HitTime(test) - HitTime(ref) [ns]", 50, -50.0, 50.0},
        {"dvx", "Vx(test) - Vx(ref) [cm]", 50, -20.0, 20.0},
        {"dvy", "Vy(test) - Vy(ref) [cm]", 50, -20.0, 20.0},
        {"dvz", "Vz(test) - Vz(ref) [cm]", 50, -40.0, 40.0},
        {"openang", "opening angle [deg]", 50, 0.0, 60.0},
        {"dsec", "DeltaSector", 13, -0.5, 12.5},
    };

    // Base directory under which ALL FDNeutralVeto debug histograms are created.
    ...
    TH1D* GetStep1D(const std::string& stage, const std::string& name, const std::string& title, int nb, double lo, double hi) {
        if (!base) return nullptr;
        TDirectory* d = MkdirPath(base, "steps/" + stage);
    ...
    TH2D* GetStep2D(const std::string& stage, const std::string& name, const std::string& title, int nbx, double lox, double hix, int nby, double loy, double hiy) {
        if (!base) return nullptr;
        TDirectory* d = MkdirPath(base, "steps2d/" + stage);
        const std::string key = d->GetPath() + std::string("/") + name;
        auto it = step2d.find(key);
        if (it != step2d.end()) return it->second;
        d->cd();
...
void FDNeutralVeto::BookIfNeeded() {
    ...
    m_dbg->n_p = new TH1D("n_p", "Neutral p(TOF) [GeV]", 50, 0.0, 10.0);
    m_dbg->n_theta = new TH1D("n_theta", "Neutral theta(TOF) [deg]", 50, 0.0, 60.0);
    m_dbg->n_phi = new TH1D("n_phi", "Neutral phi(TOF) [deg]", 50, -180.0, 180.0);
    m_dbg->n_time = new TH1D("n_time", "Neutral Time [ns]", 50, -50.0, 150.0);
    m_dbg->n_hittime = new TH1D("n_hittime", "Neutral HitTime [ns]", 50, -50.0, 150.0);
    m_dbg->n_vx = new TH1D("n_vx", "Neutral Vx [cm]", 50, -20.0, 20.0);
    m_dbg->n_vy = new TH1D("n_vy", "Neutral Vy [cm]", 50, -20.0, 20.0);
    m_dbg->n_vz = new TH1D("n_vz", "Neutral Vz [cm]", 50, -40.0, 40.0);

    m_dbg->c_p = new TH1D("c_p", "Charged p [GeV]", 50, 0.0, 10.0);
    m_dbg->c_theta = new TH1D("c_theta", "Charged theta [deg]", 50, 0.0, 60.0);
    m_dbg->c_phi = new TH1D("c_phi", "Charged phi [deg]", 50, -180.0, 180.0);
    m_dbg->c_time = new TH1D("c_time", "Charged Time [ns]", 50, -50.0, 150.0);
    m_dbg->c_hittime = new TH1D("c_hittime", "Charged HitTime [ns]", 50, -50.0, 150.0);
    m_dbg->c_vx = new TH1D("c_vx", "Charged Vx [cm]", 50, -20.0, 20.0);
    m_dbg->c_vy = new TH1D("c_vy", "Charged Vy [cm]", 50, -20.0, 20.0);
    m_dbg->c_vz = new TH1D("c_vz", "Charged Vz [cm]", 50, -40.0, 40.0);

    // Post veto neutrals
    m_dbg->n_post_p = new TH1D("n_post_p", "Neutral post veto p(TOF) [GeV]", 50, 0.0, 10.0);
    m_dbg->n_post_theta = new TH1D("n_post_theta", "Neutral post veto theta(TOF) [deg]", 50, 0.0, 60.0);
    m_dbg->n_post_phi = new TH1D("n_post_phi", "Neutral post veto phi(TOF) [deg]", 50, -180.0, 180.0);
    m_dbg->n_post_time = new TH1D("n_post_time", "Neutral post veto Time [ns]", 50, -50.0, 150.0);
    m_dbg->n_post_hittime = new TH1D("n_post_hittime", "Neutral post veto HitTime [ns]", 50, -50.0, 150.0);
    m_dbg->n_post_vx = new TH1D("n_post_vx", "Neutral post veto Vx [cm]", 50, -20.0, 20.0);
    m_dbg->n_post_vy = new TH1D("n_post_vy", "Neutral post veto Vy [cm]", 50, -20.0, 20.0);
    m_dbg->n_post_vz = new TH1D("n_post_vz", "Neutral post veto Vz [cm]", 50, -40.0, 40.0);
...
void FDNeutralVeto::FillGeneralHits_PreVeto(const std::vector<NeutralHit>& nHits, const std::vector<ChargedHit>& cHits) {
    ...
    for (const auto& h : nHits) {
        auto [tof, beta, gamma, pvec] = Calc_nFD_vars(h, m_starttime);
        m_dbg->n_p->Fill(pvec.Mag());
        m_dbg->n_theta->Fill(pvec.Theta() * TMath::RadToDeg());
        m_dbg->n_phi->Fill(pvec.Phi() * TMath::RadToDeg());
...
void FDNeutralVeto::FillGeneralHits_PostVeto(const std::vector<NeutralHit>& nSurvivors) {
    ...
    for (const auto& h : nSurvivors) {
        auto [tof, beta, gamma, pvec] = Calc_nFD_vars(h, m_starttime);
        m_dbg->n_post_p->Fill(pvec.Mag());
        m_dbg->n_post_theta->Fill(pvec.Theta() * TMath::RadToDeg());
        m_dbg->n_post_phi->Fill(pvec.Phi() * TMath::RadToDeg());
...
void FDNeutralVeto::FillStepPair_NeutNeut(const std::string& stage, const NeutralHit& test, const NeutralHit& ref) {
    ...
    auto [tof_t, beta_t, gamma_t, pt] = Calc_nFD_vars(test, m_starttime);
    auto [tof_r, beta_r, gamma_r, pr] = Calc_nFD_vars(ref, m_starttime);

    const double dp = pt.Mag() - pr.Mag();
    const double dtheta = (pt.Theta() - pr.Theta()) * TMath::RadToDeg();
    const double dphi = WrapPhiDeg((pt.Phi() - pr.Phi()) * TMath::RadToDeg());
    ...
    const double dsec = static_cast<double>(SectorDistance(test.sector, ref.sector));

    m_dbg->GetStep1D(stage, "dp", "Momentum difference [GeV]", 50, -5.0, 5.0)->Fill(dp);
    m_dbg->GetStep1D(stage, "dtheta", "Theta difference [deg]", 50, -60.0, 60.0)->Fill(dtheta);
    m_dbg->GetStep1D(stage, "dphi", "Phi difference [deg]", 50, -180.0, 180.0)->Fill(dphi);
    m_dbg->GetStep1D(stage, "dtime", "Time difference [ns]", 50, -50.0, 50.0)->Fill(dtime);
    m_dbg->GetStep1D(stage, "dhittime", "HitTime difference [ns]", 50, -50.0, 50.0)->Fill(dhittime);
    m_dbg->GetStep1D(stage, "dvx", "Vx difference [cm]", 50, -20.0, 20.0)->Fill(dvx);
    m_dbg->GetStep1D(stage, "dvy", "Vy difference [cm]", 50, -20.0, 20.0)->Fill(dvy);
    m_dbg->GetStep1D(stage, "dvz", "Vz difference [cm]", 50, -40.0, 40.0)->Fill(dvz);
    m_dbg->GetStep1D(stage, "openang", "Opening angle [deg]", 50, 0.0, 60.0)->Fill(openang);
    m_dbg->GetStep1D(stage, "dsec", "Sector difference", 13, -0.5, 12.5)->Fill(dsec);

    if (m_dbgCfg.book2DStepHists) {
        // Fill all unique 2D pairs, example subset shown; you can expand similarly
        m_dbg->GetStep2D(stage, "dp_vs_openang", "dp vs opening angle", 100, -5.0, 5.0, 100, 0.0, 60.0)->Fill(dp, openang);
        m_dbg->GetStep2D(stage, "dp_vs_dhittime", "dp vs dHitTime", 100, -5.0, 5.0, 100, -50.0, 50.0)->Fill(dp, dhittime);
        m_dbg->GetStep2D(stage, "openang_vs_dhittime", "opening angle vs dHitTime", 100, 0.0, 60.0, 100, -50.0, 50.0)->Fill(openang, dhittime);
        m_dbg->GetStep2D(stage, "dtheta_vs_dphi", "dtheta vs dphi", 100, -60.0, 60.0, 100, -180.0, 180.0)->Fill(dtheta, dphi);
        m_dbg->GetStep2D(stage, "dsec_vs_openang", "dSec vs opening angle", 13, -0.5, 12.5, 100, 0.0, 60.0)->Fill(dsec, openang);
    }
}
...
void FDNeutralVeto::FillStepPair_NeutCh(const std::string& stage, const NeutralHit& test, const ChargedHit& ref) {
    ...
    // Charged momentum from track
    const TVector3 pr = ref.P;
...
void FDNeutralVeto::FillStepSurvivor(const std::string& stage, const NeutralHit& h) {
    ...
    auto [tof, beta, gamma, pvec] = Calc_nFD_vars(h, m_starttime);
    m_dbg->GetStep1D(stage, "surv_p", "Survivor p(TOF) [GeV]", 50, 0.0, 10.0)->Fill(pvec.Mag());
    m_dbg->GetStep1D(stage, "surv_theta", "Survivor theta(TOF) [deg]", 50, 0.0, 60.0)->Fill(pvec.Theta() * TMath::RadToDeg());
    m_dbg->GetStep1D(stage, "surv_phi", "Survivor phi(TOF) [deg]", 50, -180.0, 180.0)->Fill(pvec.Phi() * TMath::RadToDeg());
    m_dbg->GetStep1D(stage, "surv_time", "Survivor Time [ns]", 50, -50.0, 150.0)->Fill(h.Time);
    m_dbg->GetStep1D(stage, "surv_hittime", "Survivor HitTime [ns]", 50, -50.0, 150.0)->Fill(h.HitTime);
    m_dbg->GetStep1D(stage, "surv_vx", "Survivor Vx [cm]", 50, -20.0, 20.0)->Fill(h.Vtx.X());
    m_dbg->GetStep1D(stage, "surv_vy", "Survivor Vy [cm]", 50, -20.0, 20.0)->Fill(h.Vtx.Y());
    m_dbg->GetStep1D(stage, "surv_vz", "Survivor Vz [cm]", 50, -40.0, 40.0)->Fill(h.Vtx.Z());
}
...
#pragma region /* AbortWithContext_GenerateHits (private helper) */

[[noreturn]] inline void FDNeutralVeto::AbortWithContext_GenerateHits(const std::string& message, const char* file, int line,
#ifdef __GNUC__
                                                                      const char* func
#else
                                                                      const char* func
...
void FDNeutralVeto::FillStepVetoed(const std::string& stage, const NeutralHit& h) {
    ...
    auto [tof, beta, gamma, pvec] = Calc_nFD_vars(h, m_starttime);
    m_dbg->GetStep1D(stage, "veto_p", "Vetoed p(TOF) [GeV]", 50, 0.0, 10.0)->Fill(pvec.Mag());
    m_dbg->GetStep1D(stage, "veto_theta", "Vetoed theta(TOF) [deg]", 50, 0.0, 60.0)->Fill(pvec.Theta() * TMath::RadToDeg());
    m_dbg->GetStep1D(stage, "veto_phi", "Vetoed phi(TOF) [deg]", 50, -180.0, 180.0)->Fill(pvec.Phi() * TMath::RadToDeg());
    m_dbg->GetStep1D(stage, "veto_time", "Vetoed Time [ns]", 50, -50.0, 150.0)->Fill(h.Time);
    m_dbg->GetStep1D(stage, "veto_hittime", "Vetoed HitTime [ns]", 50, -50.0, 150.0)->Fill(h.HitTime);
    m_dbg->GetStep1D(stage, "veto_vx", "Vetoed Vx [cm]", 50, -20.0, 20.0)->Fill(h.Vtx.X());
    m_dbg->GetStep1D(stage, "veto_vy", "Vetoed Vy [cm]", 50, -20.0, 20.0)->Fill(h.Vtx.Y());
    m_dbg->GetStep1D(stage, "veto_vz", "Vetoed Vz [cm]", 50, -40.0, 40.0)->Fill(h.Vtx.Z());
}
...
#pragma region /* Step 1: FTOF neutral veto */
        ...
        if (Apply_FTOF_Neut_Veto) {
            for (size_t j = 0; j < FTOF_neut_hits.size(); j++) {
                ...
                const int FTOF_dSec = SectorDistance(Test_Layer_sector, ref->sector);
...
#pragma region /* Step 2: charged veto */
        ...
        if (Apply_Charged_Veto) {
            for (size_t j = 0; j < Ch_hits.size(); j++) {
                ...
                const int Charged_dSec = SectorDistance(Test_Layer_sector, ref->sector);
...
#pragma region /* Step 3: earlier-ECAL neutral veto + bookkeeping */
        ...
        if (Apply_ECAL_Earliest_Layer_Veto) {
            for (size_t j = 0; j < Mid_neut_hits.size(); j++) {
                ...
                const int Neutral_dSec = SectorDistance(Test_Layer_sector, ref->sector);
...
#pragma endregion
        ...
        if (test->layer == 1 && test->num_of_ECAL_hits_ahead >= 2) { PassesIndirectTopologyCut = !ApplyReformattedStep5_IndirectThreeLayerRejection(*test); }
