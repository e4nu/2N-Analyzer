...
//
// NeutralHitStruct.cpp
// -------------------
// A lightweight POD-like struct used to store information about a *neutral* FD-associated
...
#ifndef NEUTRON_HIT  // Include guard: NeutralHit definition (header-style)
#define NEUTRON_HIT  // Include guard: NeutralHit definition (header-style)

#include <cstdlib>   // std::exit
#include <iomanip>   // std::setw, std::setprecision, std::fixed/scientific
...
// Container for a single neutral-particle hit associated with FD detectors (FTOF/ECAL).
/// \struct NeutralHit
/// \brief POD-like container for a single FD-associated *neutral* detector hit (FTOF/ECAL) plus cached vectors.
///
/// Stores both scalar values (xHit/yHit/zHit, etc.) and cached ROOT TVector3 objects (HitPos, P, Vtx, HitPath)
/// to make downstream vector algebra cleaner and avoid repeated SetXYZ calls.
///
/// Filled from a clas12root `region_part_ptr` and its sci/cal banks for a selected layer.
/// Error handling is intentionally strict: on any inconsistent/unexpected content, it prints context + hit dump
/// and aborts to protect analysis validity during debugging/validation.
struct NeutralHit {
    ...
    // Neutron hit coordinates of region_part_ptr particle in the relevant detector:
    ...
    // Path variables:
    // path = Path of region_part_ptr particle
    ...
    // Deposited energy of region_part_ptr particle in the ECAL:
    double Edep_ECAL;              // Total ECAL deposited energy of region_part_ptr particle (GeV)
    double Edep_ECAL_clas12layer;  // ECAL deposited energy of region_part_ptr particle in fired layer (GeV)

    double Vx_e, Vy_e, Vz_e;  // Electron vertex coordinates (cm)
    TVector3 Vtx;             // Cached electron vertex position (cm)

    double path, xHitPath, yHitPath, zHitPath;  // Vector from vertex to hit (cm) and its components
    TVector3 HitPath;                           // Cached (HitPos - Vtx) vector (cm)

    // Number of NeutralHit with ECAL hits ahead this one:
    // This member is filled in the AddNeutrons() function.
    int num_of_ECAL_hits_ahead;  // Bookkeeping / ordering metadata (higher-level logic)

    // Number of ECAL hits of region_part_ptr particle:
    int num_of_ECAL_hits;  // Count of ECAL hits in event/context (higher-level logic)

    bool blocking = false;  // Veto decision flag set by higher-level logic

    // TVector3 accessors (prefer these in downstream code)
    ...
    /// \details This is intended to make uninitialized usage obvious in debug printouts.
    void reset() {
        // Common to both NeutralHit and ChargedHit:
        // ------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        Edep_ECAL = -999.;              // Default invalid deposited energy
        Edep_ECAL_clas12layer = -999.;  // Default invalid deposited energy

        Vx_e = -999., Vy_e = -999., Vz_e = -999.;  // Default invalid electron vertex
        Vtx.SetXYZ(Vx_e, Vy_e, Vz_e);              // Keep vector cache consistent

        num_of_ECAL_hits_ahead = 0;  // Reset ordering metadata
        num_of_ECAL_hits = 0;        // Reset ECAL hit count metadata

        blocking = false;  // Reset veto flag
    }

    /// \brief Fill this NeutralHit from a reconstructed particle and a selected detector layer.
    ...
    /// \param Vtx_e electron vertex used to build HitPath = HitPos - Vtx_e.
    void FillHitData(region_part_ptr& Particle, int i, int Hitnum, int Detector, int Layer, short clas12detlayer, const bool PCAL_hit, const bool ECIN_hit, const bool ECOUT_hit,
                     TVector3 Vtx_e, int Num_of_ECAL_hits) {
        bool printOut = false;
        ...
        Edep_ECAL = Particle->cal(clas12::PCAL)->getEnergy() + Particle->cal(clas12::ECIN)->getEnergy() + Particle->cal(clas12::ECOUT)->getEnergy();

        Edep_ECAL_clas12layer = Particle->cal(clas12detlayer)->getEnergy();

        // Vertex (electron) (cache as TVector3 and keep scalars in sync)
        Vtx = Vtx_e;                                     // Cache electron vertex vector
        Vx_e = Vtx.X(), Vy_e = Vtx.Y(), Vz_e = Vtx.Z();  // Cache scalar components

        // Path length and hit-path vector (hit position relative to the provided vertex)
        HitPath = HitPos - Vtx_e;                                                // Build vertex->hit vector
        path = HitPath.Mag();                                                    // Cache magnitude
        xHitPath = HitPath.X(), yHitPath = HitPath.Y(), zHitPath = HitPath.Z();  // Cache components

        num_of_ECAL_hits = Num_of_ECAL_hits;
        ...
        // // TODO: I'm getting that some Lv/Lw can be zero in some rare cases. Check why this happens.
        // if (detector == 7) {
        //     // if (Lv <= 0) {
        //     //     std::ostringstream oss;
        //     //     oss << "FillHitData: invalid Lv (" << Lv << ")";
        //     //     AbortWithContext(oss.str());
        //     // }  // Validate
        //     // if (Lw <= 0) {
        //     //     std::ostringstream oss;
        //     //     oss << "FillHitData: invalid Lw (" << Lw << ")";
        //     //     AbortWithContext(oss.str());
        //     // }  // Validate
        // }
        ...
        // Energy fields are expected to be non-negative for ECAL-associated content.
        if (detector == 7 && Edep_ECAL < 0.) {                              // Validate total ECAL deposited energy (should be >= 0 for ECAL)
            std::ostringstream oss;                                         // Build a detailed error message
            oss << "FillHitData: invalid Edep_ECAL (" << Edep_ECAL << ")";  // Include the offending value
            AbortWithContext(oss.str());                                    // Abort with context + dump
        }  // Validate

        if (detector == 7 && Edep_ECAL_clas12layer < 0.) {                                          // Validate deposited energy in the fired layer (should be >= 0 for ECAL)
            std::ostringstream oss;                                                                 // Build a detailed error message
            oss << "FillHitData: invalid Edep_ECAL_clas12layer (" << Edep_ECAL_clas12layer << ")";  // Include the offending value
            AbortWithContext(oss.str());                                                            // Abort with context + dump
        }  // Validate
    }
    ...
    /// \brief Build the standard NeutralHit key/value dump items (used by PrintOut and DebugPrintOut).
    ...
    std::vector<std::pair<std::string, std::string>> BuildPrintOutItems() const {
        ...
        }
        {
            ...
            items.emplace_back("{detector, sector, layer}", oss.str());  // Detector identifiers
        }
        {
            std::ostringstream oss;
            oss << "{" << bt::BoolToString(clas12particle_hit_in_PCAL) << ", " << bt::BoolToString(clas12particle_hit_in_ECIN) << ", " << bt::BoolToString(clas12particle_hit_in_ECOUT)
                << "}";
            items.emplace_back("{PCAL hit, ECIN hit, ECOUT hit}", oss.str());  // ECAL hit flags
        }

        items.emplace_back("First_ECAL_clas12detlayer", First_ECAL_clas12detlayer_str);
        items.emplace_back("particle_clas12detlayer", particle_clas12detlayer_str);

        {
            std::ostringstream oss;
            oss << "{" << Lv << " [cm], " << Lw << " [cm]}";
            items.emplace_back("{Lv, Lw}", oss.str());
        }
        {
            std::ostringstream oss;
            oss << "{" << HitPos.Mag() << " [cm], " << HitPos.Theta() * TMath::RadToDeg() << " [deg], " << HitPos.Phi() * TMath::RadToDeg() << " [deg]}";
            items.emplace_back("{|HitPos|, Theta_HitPos, Phi_HitPos}", oss.str());
        }
        {
            std::ostringstream oss;
            oss << HitTime << " [ns]";
            items.emplace_back("HitTime", oss.str());
        }
        {
            std::ostringstream oss;
            oss << Time << " [ns]";
            items.emplace_back("Time", oss.str());
        }
        {
            std::ostringstream oss;
            oss << clas12path << " [cm]";
            items.emplace_back("clas12path", oss.str());
        }
        {
            std::ostringstream oss;
            oss << Edep_ECAL << " [GeV]";
            items.emplace_back("Edep_ECAL", oss.str());
        }
        {
            std::ostringstream oss;
            oss << Edep_ECAL_clas12layer << " [GeV]";
            items.emplace_back("Edep_ECAL_clas12layer", oss.str());
        }
        {
            std::ostringstream oss;
            oss << "{" << P.Mag() << " [GeV/c], " << P.Theta() * TMath::RadToDeg() << " [deg], " << P.Phi() * TMath::RadToDeg() << " [deg]}";
            items.emplace_back("{|P|, Theta_P, Phi_P}", oss.str());
        }
        {
            std::ostringstream oss;
            oss << "{" << HitPath.Mag() << " [cm], " << HitPath.Theta() * TMath::RadToDeg() << " [deg], " << HitPath.Phi() * TMath::RadToDeg() << " [deg]}";
            items.emplace_back("{|HitPath|, Vtx_HitPath, Vtx_HitPath}", oss.str());
        }
        {
            std::ostringstream oss;
            oss << num_of_ECAL_hits_ahead;
            items.emplace_back("num_of_ECAL_hits_ahead", oss.str());
        }
        {
            std::ostringstream oss;
            oss << num_of_ECAL_hits;
            items.emplace_back("num_of_ECAL_hits", oss.str());
        }

        items.emplace_back("blocking", bt::BoolToString(blocking));
    ...
    void PrintOut(std::string PrintColor = "\033[34m", bool DebugPrint = false) {
    ...
    /// \param PrintColor ANSI color escape string (e.g., "\033[34m").
    ...
    static void PrintAlignedKV(const std::vector<std::pair<std::string, std::string>>& items, const std::string& PrintColor = "\033[34m", bool resetColor = true, bool leadingNewlines = true,
    ...
    void DebugPrintOut(const std::vector<std::pair<std::string, std::string>>& extra, std::string PrintColor = "\033[34m", bool DebugPrint = false) {
    ...
    [[noreturn]] void AbortWithContext(const std::string& message, const std::source_location& loc = std::source_location::current()) const {
        ...
        const_cast<NeutralHit*>(this)->PrintOut();                                                                                              // Dump full content (PrintOut is non-const)
        std::cout << "Aborting...\n\n";                                                                                                         // Termination message
    ...
    [[noreturn]] void AbortWithContext(const std::string& message, const char* file = __FILE__, int line = __LINE__,
        ...
        std::cout << "Hit details:\n";                                                                          // Header for dump
        const_cast<NeutralHit*>(this)->PrintOut();                                                              // Dump full content
        std::cout << "Aborting...\n\n";                                                                         // Termination message
        std::exit(1);                                                                                           // Hard abort (intentional)
    }
#endif
