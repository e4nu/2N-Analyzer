...
#pragma region /* Setup */
    ...
    std::string OutFolderName = OutFolderName_prefix + OutFolderName_ver_status + Ebeam_status + "_" + samples_status + ReDef_status + cPart_ECAL_veto_status + nPart_ECAL_veto_status +
...
#pragma region /* A helper lambda to build the FD photons and the ECAL/matched FD neutrons */
        ...
        auto Build_FD_neutrals = [&]() {
            ...
            NeutralHit Local_reco_LnFD;  // Leading neutron
            std::vector<region_part_ptr> Local_gammaFD;
            std::vector<int> Local_reco_nFD_ind;

            // #pragma region /* Ron's veto with std::vector<region_part_ptr> */
            ...
            //                 // bool PassMomTh = true;
            //                 bool PassECALeadgeCuts = (Temp_local_reco_nFD[i]->cal(Neutron_ECAL_detlayer)->getLv() > 14. && Temp_local_reco_nFD[i]->cal(Neutron_ECAL_detlayer)->getLw()
            //                 > 14.);
...
#pragma region /* Ron's veto with std::vector<NeutralHit> */

            std::vector<NeutralHit> Local_reco_nFD;           // All neutrons
            std::vector<NeutralHit> Local_reco_nFD_OnlyGood;  // Only good neutrons
            std::vector<NeutralHit> Local_reco_nFD_OnlyBad;   // Only Bad neutrons

            std::vector<int> Temp_local_reco_nFD_ind;
            // std::vector<NeutralHit> Temp_local_reco_nFD = GetNewNeutronFDVetoHits(allParticles, Temp_local_reco_nFD_ind);
            std::vector<NeutralHit> Temp_local_reco_nFD = GetNewNeutronFDVetoHits(allParticles, electrons[0], Temp_local_reco_nFD_ind);

            for (int i = 0; i < Temp_local_reco_nFD.size(); i++) {
                double Momentum = analysis_math::RadCalc(Temp_local_reco_nFD[i].Px, Temp_local_reco_nFD[i].Py, Temp_local_reco_nFD[i].Pz);
                double Theta_deg = analysis_math::CalcTheta_rad(Temp_local_reco_nFD[i].xHit - electrons[0]->par()->getVx(), Temp_local_reco_nFD[i].yHit - electrons[0]->par()->getVy(),
                                                                Temp_local_reco_nFD[i].zHit - electrons[0]->par()->getVz()) *
                                   180.0 / pi;

                bool PassMomTh = (Momentum >= nFD_mom_th.GetLowerCut());
                // bool PassMomTh = true;
                // bool PassMinimalThetanFDTh = (Theta_deg >= 5.);
                bool PassMinimalThetanFDTh = true;
                ...
                bool PassECALeadgeCuts = (Temp_local_reco_nFD[i].Lv > 14. && Temp_local_reco_nFD[i].Lw > 14.);
                // bool PassECALeadgeCuts = true;

                bool PassCVeto = NeutronECAL_Cut_Veto_original_custom_2(allParticles, electrons, Ebeam, Temp_local_reco_nFD[i], cPart_veto_radius);
                // bool PassCVeto = true;
                bool PassNVeto = NeutronECAL_Cut_NeutVeto_custom_2(allParticles, electrons, Ebeam, Temp_local_reco_nFD[i], nPart_veto_radius);
                // bool PassNVeto = true;
                ...
                if (PassMomTh && PassECALeadgeCuts && PassMinimalThetanFDTh && (!apply_cPart_ECAL_veto || PassCVeto) && (!apply_nPart_ECAL_veto || PassNVeto)) {
                    ...
                    int nFD_nSector = Temp_local_reco_nFD[i].sector;
                    int e_nSector = electrons[0]->cal(Temp_local_reco_nFD[i].Neutron_ECAL_detlayer)->getSector();  // Get electron sector at the same ECAL layer as FD neutron
            ...
            int Temp_Reco_P_LnFD_ind_local = -1;

            for (size_t k = 0; k < Local_reco_nFD.size(); ++k) {
                int idx = Local_reco_nFD[k].pindex;

                double Momentum = analysis_math::RadCalc(Local_reco_nFD[k].Px, Local_reco_nFD[k].Py, Local_reco_nFD[k].Pz);

                if (Momentum >= Reco_P_LnFD_mag_local) {
                    Reco_P_LnFD_mag_local = Momentum;
                    Reco_P_LnFD_ind_local = idx;  // global index
                    Temp_Reco_P_LnFD_ind_local = k;
                }
            }
...
#pragma region /* Filling FD leading neutron plots (ECALveto) */
            ...
            if (Reco_P_LnFD_ind_ECALveto != -1) {
                ...
                int e_nSector = electrons[0]->cal(neutrons_FD_ECALveto_LnFD.Neutron_ECAL_detlayer)->getSector();  // Get electron sector at the same ECAL layer as FD neutron
                ...
                double Path_LnFD = analysis_math::RadCalc(neutrons_FD_ECALveto_LnFD.xHit, neutrons_FD_ECALveto_LnFD.yHit, neutrons_FD_ECALveto_LnFD.zHit);
                ...
                reco_P_LnFD.SetMagThetaPhi(analysis_math::RadCalc(neutrons_FD_ECALveto_LnFD.Px, neutrons_FD_ECALveto_LnFD.Py, neutrons_FD_ECALveto_LnFD.Pz),
                                           allParticles[neutrons_FD_ECALveto_LnFD.pindex]->getTheta(), allParticles[neutrons_FD_ECALveto_LnFD.pindex]->getPhi());
                // analysis_math::CalcTheta_rad(neutrons_FD_ECALveto_LnFD.xHit - electrons[0]->par()->getVx(), neutrons_FD_ECALveto_LnFD.yHit - electrons[0]->par()->getVy(),
                //                              neutrons_FD_ECALveto_LnFD.zHit - electrons[0]->par()->getVz()),
                // analysis_math::CalcPhi_rad(neutrons_FD_ECALveto_LnFD.xHit - electrons[0]->par()->getVx(), neutrons_FD_ECALveto_LnFD.yHit - electrons[0]->par()->getVy()));
                ...
                for (int i = 0; i < neutrons_FD_ECALveto.size(); i++) {
                    if (analysis_math::RadCalc(neutrons_FD_ECALveto[i].Px, neutrons_FD_ECALveto[i].Py, neutrons_FD_ECALveto[i].Pz) > reco_P_LnFD.Mag() &&
...
#pragma region /* Filling FD neutron plots (ECALveto) */
            ...
            for (int i = 0; i < neutrons_FD_ECALveto.size(); i++) {
                ...
                int e_nSector = electrons[0]->cal(neutrons_FD_ECALveto[i].Neutron_ECAL_detlayer)->getSector();  // Get electron sector at the same ECAL layer as FD neutron
                ...
                double Path_nFD = analysis_math::RadCalc(neutrons_FD_ECALveto[i].xHit, neutrons_FD_ECALveto[i].yHit, neutrons_FD_ECALveto[i].zHit);
                ...
                auto [PCAL_hit_test, ECIN_hit_test, ECOUT_hit_test, ECAL_detlayer_test] = reco_analysis_functions::CheckForECALHits(allParticles[neutrons_FD_ECALveto[i].pindex]);
                ...
                TVector3 reco_P_nFD;
                reco_P_nFD.SetMagThetaPhi(analysis_math::RadCalc(neutrons_FD_ECALveto[i].Px, neutrons_FD_ECALveto[i].Py, neutrons_FD_ECALveto[i].Pz),
                                          allParticles[neutrons_FD_ECALveto[i].pindex]->getTheta(), allParticles[neutrons_FD_ECALveto[i].pindex]->getPhi());
                //   analysis_math::CalcTheta_rad(neutrons_FD_ECALveto[i].xHit - electrons[0]->par()->getVx(), neutrons_FD_ECALveto[i].yHit - electrons[0]->par()->getVy(),
                //                                neutrons_FD_ECALveto[i].zHit - electrons[0]->par()->getVz()),
                //   analysis_math::CalcPhi_rad(neutrons_FD_ECALveto[i].xHit - electrons[0]->par()->getVx(), neutrons_FD_ECALveto[i].yHit - electrons[0]->par()->getVy()));
...
#pragma region /* Fill leading FD neutron plots and matching (matched) */
            ...
            if (Reco_P_LnFD_ind_matched != -1) {
                for (int j = 0; j < truth_NeutronsFD.size(); j++) {
                    ...
                    TVector3 reco_P_LnFD;
                    reco_P_LnFD.SetMagThetaPhi(analysis_math::RadCalc(neutrons_FD_matched_LnFD.Px, neutrons_FD_matched_LnFD.Py, neutrons_FD_matched_LnFD.Pz),
                                               allParticles[neutrons_FD_matched_LnFD.pindex]->getTheta(), allParticles[neutrons_FD_matched_LnFD.pindex]->getPhi());
                    // analysis_math::CalcTheta_rad(neutrons_FD_matched_LnFD.xHit - electrons[0]->par()->getVx(), neutrons_FD_matched_LnFD.yHit - electrons[0]->par()->getVy(),
                    //                              neutrons_FD_matched_LnFD.zHit - electrons[0]->par()->getVz()),
                    // analysis_math::CalcPhi_rad(neutrons_FD_matched_LnFD.xHit - electrons[0]->par()->getVx(), neutrons_FD_matched_LnFD.yHit - electrons[0]->par()->getVy()));
...
#pragma region /* Filling FD neutron plots (matched) */
            ...
            for (int i = 0; i < neutrons_FD_matched.size(); i++) {
                ...
                int e_nSector = electrons[0]->cal(neutrons_FD_matched[i].Neutron_ECAL_detlayer)->getSector();  // Get electron sector at the same ECAL layer as FD neutron
                if (OnlyGood_nFD) { nFD_pass_dSector_cuts = (abs(nFD_nSector - e_nSector) == 3); }
                if (OnlyBad_nFD) { nFD_pass_dSector_cuts = !(abs(nFD_nSector - e_nSector) == 3); }
                ...
                double Path_nFD = analysis_math::RadCalc(neutrons_FD_matched[i].xHit, neutrons_FD_matched[i].yHit, neutrons_FD_matched[i].zHit);
                ...
                TVector3 reco_P_nFD;
                reco_P_nFD.SetMagThetaPhi(analysis_math::RadCalc(neutrons_FD_matched[i].Px, neutrons_FD_matched[i].Py, neutrons_FD_matched[i].Pz),
                                          allParticles[neutrons_FD_matched[i].pindex]->getTheta(), allParticles[neutrons_FD_matched[i].pindex]->getPhi());
                //   analysis_math::CalcTheta_rad(neutrons_FD_matched[i].xHit - electrons[0]->par()->getVx(), neutrons_FD_matched[i].yHit - electrons[0]->par()->getVy(),
                //                                neutrons_FD_matched[i].zHit - electrons[0]->par()->getVz()),
                //   analysis_math::CalcPhi_rad(neutrons_FD_matched[i].xHit - electrons[0]->par()->getVx(), neutrons_FD_matched[i].yHit - electrons[0]->par()->getVy()));
