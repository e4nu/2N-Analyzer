...
class RecoAnalyzer {
   ...
   private:
    ...
    void InitializeCutSetup(AnalysisCutSettings& CutSettings, std::string& run_plots_path, std::string& run_plots_log_save_Directory, std::string& run_skipped_files_list_save_Directory,
                            Settings& settings, AcceptanceMapsSettings& AMapsSettings, MomentumResolutionSettings& MomResSettings, EventSelectionSettings& ESSettings,
                            const RunParameters& parameters, const std::string& RecoSamplePath, const std::string& ReconHipoDir, const std::string& InputHipoFiles) const;
    ...
    void InitializeHistogramSettings(HistogramSettings& HistSettings, const EventSelectionSettings& ESSettings, const AcceptanceMapsSettings& AMapsSettings,
                                     const AnalysisCutSettings& CutSettings, const RunParameters& parameters, const CutValueManager& CutManager, const char* caller_function) const;

    void InitializeAcceptanceMaps(AcceptanceMapsSettings& AMapsSettings, const EventSelectionSettings& ESSettings, HistogramSettings& HistSettings, const RunParameters& parameters,
                                  Directories& directories, AMaps& aMaps_master, AMaps& wMaps_master, std::vector<std::vector<double>>& ElectronMomSliceLimits,
                                  std::vector<std::vector<double>>& NucleonMomSliceLimits) const;
    ...
    void InitializeMomentumResolution(MomentumResolutionSettings& MomResSettings, const EventSelectionSettings& ESSettings, const AnalysisCutSettings& CutSettings,
                                      const RunParameters& parameters, const CutValueManager& CutManager, Directories& directories, MomentumResolution& nRes, MomentumResolution& pRes) const;

    void InitializeDebuggingSetup(ParticleID& pid, std::ofstream& EventPrint, std::string& EventPrint_save_Directory, Directories& directories, const AnalysisCutSettings& CutSettings,
