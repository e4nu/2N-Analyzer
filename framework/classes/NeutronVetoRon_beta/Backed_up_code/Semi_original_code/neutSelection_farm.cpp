...
#include "/u/home/rwagner/clas12/Software/git_code/dir_latest/PhD/analysis_code/neutron_veto_code/ChargedHit.h"
#include "/u/home/rwagner/clas12/Software/git_code/dir_latest/PhD/analysis_code/neutron_veto_code/neutronHitStruct_local.cpp"

// should retur vector of int that point to hit indexes that should be considered as neutrons
/*
struct NeutronHit{
    int hitIndex;
    float x;
...
void AddNeutrons(std::vector<NeutronHit*> tof_n, std::vector<NeutronHit*> mid_n, std::vector<ChargedHit> vChrg, std::vector<NeutronHit*>& testLay, std::vector<NeutronHit*>& neut) {
    ...
    */
    ...
    for (int i = 0; i < testLay.size(); i++) {
        ...
        // check if there are neutral hits in the same sector near each ather
...
std::vector<NeutronHit*> neutSelection(std::vector<NeutronHit*> nHit, std::vector<ChargedHit> cHit) {
    ...
    // introduce tempor vector that combine ALL preciding planes to be used as blocking layer. This is done to avoid cases when we have hit in layer 1 and 3, without hit in layer 2.
