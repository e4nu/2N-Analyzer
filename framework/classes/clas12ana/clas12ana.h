...
void clas12ana::Run(const std::unique_ptr<clas12::clas12reader> &c12) {
    ...
    if (electrons_det.size() == 1) {
        ...
        //        #pragma region /* Debugging print - START */
        ...
        if (debug_plots) {
            ...
            hTheta_e_vs_Phi_e_fiducial_cuts_map_ECAL->Fill(electrons_det[0]->getPhi() * 180.0 / pi, electrons_det[0]->getTheta() * 180.0 / pi);
            // My addition
        ...
        //        #pragma region /* Debugging print - END */
...
bool clas12ana::checkEcalSFCuts(region_part_ptr p) {
    ...
    //    #pragma region /* Justin's original */
    //    if (p->par()->getPid() == 11) {
    //        double sampling_frac = getSF(p);
    ...
    if (infile.is_open()) {
                       ...
                       tp))  // read data from file object and put it into string.
        {
            ...
            if (parameter == "pid_cuts") {
                ...
                if (pid != "") vertex_cuts.insert(pair<string, vector<double> >(pid, par));
...
TVector3 clas12ana::getCOM(TLorentzVector lead, TLorentzVector recoil, TLorentzVector q) {
    ...
    if (abs(com.Dot(vy)) < 20) cout << com.Mag() << " " << com.Dot(vx) << " " << com.Dot(vy) << " " << com.Dot(vz) << " " << lead.P() << " " << recoil.P() << endl;
