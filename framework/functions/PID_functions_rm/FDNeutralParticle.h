...
void ReDefFDNeutrals(vector<region_part_ptr> allParticles, vector<region_part_ptr> electrons, vector<int> &ID_Neutrons_FD, vector<int> &ID_Photons_FD, DSCuts &Neutron_veto_cut,
                     const double &beamE) {
    for (int i = 0; i < allParticles.size(); i++) {
        int ParticlePDG = allParticles[i]->par()->getPid();
