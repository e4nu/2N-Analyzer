...
void GetTruthLevelCuts(const std::string &SampleName, bool calculate_truth_level, bool Enable_FD_photons, bool apply_nucleon_cuts, TH2D *Electron_AMap, TH2D *Proton_AMap, TH2D *Neutron_AMap,
                       const DSCuts &ThetaFD, double Theta_ulim_FD, double Theta_llim_FD, const DSCuts &ThetaCD, double Theta_ulim_CD, double Theta_llim_CD,
                       double Phi_llim, double Phi_ulim, const DSCuts &TL_e_mom_cuts, const DSCuts &TL_n_mom_cuts, const DSCuts &TL_p_mom_cuts, const DSCuts &TL_pip_mom_cuts,
                       const DSCuts &TL_pim_mom_cuts, const DSCuts &TL_pi0_mom_cuts, const DSCuts &TL_ph_mom_cuts, const std::unique_ptr<clas12::clas12reader> &c12) {
    if (calculate_truth_level && findSubstring(SampleName, "sim") && apply_nucleon_cuts) {  // run only for CLAS12 simulation & AFTER beta fit
        ...
        for (Int_t i = 0; i < Ngen; i++) {
            ...
            int BinX = GetBinFromAng(Particle_TL_Phi, Neutron_AMap->GetNbinsX(), Phi_llim, Phi_ulim, false, "Phi");
            int BinY = GetBinFromAng(Particle_TL_Theta, Neutron_AMap->GetNbinsY(), Theta_llim_FD, Theta_ulim_FD, false, "Theta");
            int BinX_e = GetBinFromAng(Particle_TL_Phi, Electron_AMap->GetNbinsX(), Phi_llim, Phi_ulim, false, "Phi");
            int BinY_e = GetBinFromAng(Particle_TL_Theta, Electron_AMap->GetNbinsY(), Theta_llim_FD, Theta_ulim_FD, false, "Theta");
            int BinX_p = GetBinFromAng(Particle_TL_Phi, Proton_AMap->GetNbinsX(), Phi_llim, Phi_ulim, false, "Phi");
            int BinY_p = GetBinFromAng(Particle_TL_Theta, Proton_AMap->GetNbinsY(), Theta_llim_FD, Theta_ulim_FD, false, "Theta");
        ...
        bool TL_Event_Selection_1e_cut = (TL_Electron_mom_ind.size() == 1 && TL_ElectronFD_mom_ind.size() == 1);  // One id. electron above momentum threshold
