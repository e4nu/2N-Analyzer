...
#include "setup/path_definitions.h"
...
struct AnalysisCutSettings {
    ...
    bool custom_cuts_naming;  // Enable custom cuts naming
    ...
    void UpdateCutSettings(const RunParameters& parameters, EventSelectionSettings& ESSettings, AcceptanceMapsSettings& AMapsSettings, MomentumResolutionSettings& MomResSettings) {
        ...
        if (ESSettings.Rec_wTL_ES) {
            ...
            /*
                Safety measure:
                If ESSettings.Rec_wTL_ES = false, keep fiducial cuts with the overlapping maps!
                This safety measure is only relevant when NOT calculating momentum resolution.
                For momentum calculations, the Calc_eff_overlapping_FC = false option gives more statistics.
            */
            if (!MomResSettings.plot_and_fit_MomRes) { ESSettings.Calc_eff_overlapping_FC = true; }
        }
    ...
    void UpdateCustomNaming(Settings& settings, const AcceptanceMapsSettings& AMapsSettings, const MomentumResolutionSettings& MomResSettings, const EventSelectionSettings& ESSettings,
        ...
        settings.ConfigureStatuses(*this, ESSettings, AMapsSettings, MomResSettings);

        settings.SetPaths(pd::PathDefinitions.WorkingDirectory, parameters.SampleName, run_plots_path, apply_cuts, apply_chi2_cuts_1e_cut, apply_nucleon_cuts);
    ...
    void SetClas12anaCuts(clas12ana& clasAna, CutValueManager& CutManager, const RunParameters& parameters, const EventSelectionSettings& ESSettings) {
        if (apply_cuts) {
            ...
            if (!apply_chi2_cuts_1e_cut) {
                ...
                bt::PrintMessage("[AnalysisCutSettings::SetClas12anaCuts] Loading fitted pid cuts...");
                ...
                bt::PrintMessage("[AnalysisCutSettings::SetClas12anaCuts] done.");
            }
            ...
            if (!apply_nucleon_cuts) {
                ...
                bt::PrintMessage("[AnalysisCutSettings::SetClas12anaCuts] Loading fitted Beta cuts...");
...
// --------------------------------------------------------------------------------------
// YAML adapter specialization for AnalysisCutSettings
// --------------------------------------------------------------------------------------

template <>
struct Yaml_loader::SettingsAdapter<AnalysisCutSettings> {
    static void Apply(const YAML::Node& node, AnalysisCutSettings& s) {
        using namespace Yaml_loader;

        // Top-level cut switches
        TryReadKey(node, "apply_cuts", s.apply_cuts);
        TryReadKey(node, "clas12ana_particles", s.clas12ana_particles);
        TryReadKey(node, "only_preselection_cuts", s.only_preselection_cuts);
        TryReadKey(node, "only_electron_quality_cuts", s.only_electron_quality_cuts);

        // Preselection cuts
        TryReadKey(node, "apply_preselection_cuts", s.apply_preselection_cuts);
        TryReadKey(node, "apply_Vz_e_cuts", s.apply_Vz_e_cuts);
        TryReadKey(node, "apply_Vz_cuts", s.apply_Vz_cuts);
        TryReadKey(node, "apply_dVz_cuts", s.apply_dVz_cuts);
        TryReadKey(node, "apply_DC_e_fiducial_cuts", s.apply_DC_e_fiducial_cuts);
        TryReadKey(node, "apply_DC_fiducial_cuts", s.apply_DC_fiducial_cuts);

        // Electron quality cuts
        TryReadKey(node, "apply_electron_quality_cuts", s.apply_electron_quality_cuts);
        TryReadKey(node, "apply_Nphe_cut", s.apply_Nphe_cut);
        TryReadKey(node, "apply_ECAL_SF_cuts", s.apply_ECAL_SF_cuts);
        TryReadKey(node, "apply_ECAL_P_cuts", s.apply_ECAL_P_cuts);
        TryReadKey(node, "apply_ECAL_fiducial_cuts", s.apply_ECAL_fiducial_cuts);
        TryReadKey(node, "apply_Electron_beta_cut", s.apply_Electron_beta_cut);

        // Chi2 cuts
        TryReadKey(node, "apply_chi2_cuts_1e_cut", s.apply_chi2_cuts_1e_cut);

        // Top-level nucleon cuts switch
        TryReadKey(node, "apply_nucleon_cuts", s.apply_nucleon_cuts);

        // Nucleon PID cuts
        TryReadKey(node, "apply_nucleon_PID_cuts", s.apply_nucleon_PID_cuts);
        TryReadKey(node, "apply_CDEdgeCuts", s.apply_CDEdgeCuts);
        TryReadKey(node, "apply_CDRegionCuts", s.apply_CDRegionCuts);
        TryReadKey(node, "apply_ProtonPIDCuts", s.apply_ProtonPIDCuts);
        TryReadKey(node, "apply_GhostTrackCuts", s.apply_GhostTrackCuts);
        TryReadKey(node, "apply_charged_ECAL_veto", s.apply_charged_ECAL_veto);
        TryReadKey(node, "apply_neutral_ECAL_veto", s.apply_neutral_ECAL_veto);

        // Nucleon physical cuts
        TryReadKey(node, "apply_nucleon_physical_cuts", s.apply_nucleon_physical_cuts);
        TryReadKey(node, "apply_nBeta_fit_cuts", s.apply_nBeta_fit_cuts);
        TryReadKey(node, "apply_fiducial_cuts", s.apply_fiducial_cuts);
        TryReadKey(node, "apply_kinematical_cuts", s.apply_kinematical_cuts);
        TryReadKey(node, "apply_kinematical_weights", s.apply_kinematical_weights);
        TryReadKey(node, "apply_nucleon_SmearAndCorr", s.apply_nucleon_SmearAndCorr);

        // Reaction monitoring cuts
        TryReadKey(node, "apply_reaction_monitoring_cuts", s.apply_reaction_monitoring_cuts);
        TryReadKey(node, "apply_P_miss_in_QE_range_cuts", s.apply_P_miss_in_QE_range_cuts);
        TryReadKey(node, "apply_E_miss_in_QE_range_cuts", s.apply_E_miss_in_QE_range_cuts);
        TryReadKey(node, "apply_P_miss_in_MECandSRC_range_cuts", s.apply_P_miss_in_MECandSRC_range_cuts);
        TryReadKey(node, "apply_E_miss_in_MECandSRC_range_cuts", s.apply_E_miss_in_MECandSRC_range_cuts);
        TryReadKey(node, "apply_xB_in_QE_range_cuts", s.apply_xB_in_QE_range_cuts);
        TryReadKey(node, "apply_theta_q_pCD_in_lower_FSI_range_cut", s.apply_theta_q_pCD_in_lower_FSI_range_cut);

        // Naming
        TryReadKey(node, "custom_cuts_naming", s.custom_cuts_naming);
    }

    static void Validate(const AnalysisCutSettings& s) {
        if (s.only_preselection_cuts && s.only_electron_quality_cuts) {
            bt::ExitWithError(__func__, __FILE__, __LINE__, "only_preselection_cuts and only_electron_quality_cuts cannot both be true.");
        }
    }
};
