...
/**
 * @namespace debugging
 * Central home for analysis-wide debugging switches and the shared Debugger instance.
 *
 * Purpose:
 *   Collect the runtime flags that control optional debug output and safety monitoring,
 *   and expose a single inline `CodeDebugger` object that can be used anywhere in the
 *   analysis without passing a Debugger instance through every call chain.
 *
 * Contents:
 *   - `DebuggerMode`: master switch for step-by-step debug printing
 *   - `OnlyPrintNamedTesterSteps`: restrict step printing to calls that provide a step name
 *   - `AMapsDebuggerMode`: extra debug mode for acceptance-map / weight-map workflows
 *   - `GoodProtonsMonitorPlots`: enable proton-monitoring plots when that monitoring path is used
 *   - `PrintEvents`, `Ne_in_event`, `Nf_in_event`, `nEvents2print`: controls for selective event printing
 *   - `CodeDebugger`: shared inline Debugger object used by the analysis code
 *
 * Notes:
 *   The boolean flags in this namespace are configuration constants, not mutable runtime state.
 *   `CodeDebugger` is declared `inline` so the header can be included in multiple translation
...
class Debugger {
   ...
   public:
    ...
    // RestStepTester function
    // ------------------------------------------------------------------------------------------------------------------------------------------------------------------

    void RestStepTester() { StepTester = 1; };
    ...
    void PrintStepTester(const char* FILE, const int LINE, const bool& DebuggerMode, const bool& OnlyPrintNamedTesterSteps, const std::string& StepName = "") {
        if (DebuggerMode) {
            const std::string normalizedFile =  bt::NormalizeFilePathForPrint(FILE != nullptr ? FILE : "");
            if (OnlyPrintNamedTesterSteps) {
                if (StepName != "") {
                    ...
                    std::cerr << env::ERROR_COLOR << "File:" << env::RESET_COLOR << " " << normalizedFile << env::ERROR_COLOR << ", Line:" << env::RESET_COLOR << " " << LINE
                              << env::ERROR_COLOR << ", Step: " << env::RESET_COLOR << "" << StepName << "\n\n"
    ...
    void PrintErrorMessage(const std::string& ErrorMessage, const std::string& FunctionName = "") {
        const std::string normalizedFile =  bt::NormalizeFilePathForPrint(__FILE__);
    ...
    // PrintMissingNeutralFDECALHitWarning function --------------------------------------------------------------------------------------------------------------------

    void PrintMissingNeutralFDECALHitWarning(const char* FunctionName, const char* FILE, const int LINE, const clas12::region_part_ptr& NeutralFD, const bool& ParticleInPCAL,
                                             const bool& ParticleInECIN, const bool& ParticleInECOUT);

    // PrintMissingFTOFHitWarning function ----------------------------------------------------------------------------------------------------------------------------

    void PrintMissingFTOFHitWarning(const char* FunctionName, const char* FILE, const int LINE, const clas12::region_part_ptr& ParticleFD, const bool& ParticleInFTOF1A,
