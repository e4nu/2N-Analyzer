...
// --------------------------------------------------------------------------------------
// YAML adapter specialization for DSCuts
// --------------------------------------------------------------------------------------

template <>
struct Yaml_loader::SettingsAdapter<DSCuts> {
    static void Apply(const YAML::Node& node, DSCuts& s) {
        using namespace Yaml_loader;

        // Rebuild the metadata through the constructor so PDG bookkeeping stays consistent.
        std::string cut_variable = s.GetCutVariable();
        std::string region = s.GetRegion();
        std::string particle = s.GetPart();
        std::string applied_cuts = s.GetAppliedCuts();
        double mean = s.GetMean();
        double lower_cut = s.GetLowerCut();
        double upper_cut = s.GetUpperCut();

        bool metadata_changed = false;
        metadata_changed |= TryReadKey(node, "CutVariable", cut_variable);
        metadata_changed |= TryReadKey(node, "Region", region);
        metadata_changed |= TryReadKey(node, "Particle", particle);
        metadata_changed |= TryReadKey(node, "AppliedCuts", applied_cuts);
        metadata_changed |= TryReadKey(node, "Mean", mean);
        metadata_changed |= TryReadKey(node, "LowerCut", lower_cut);
        metadata_changed |= TryReadKey(node, "UpperCut", upper_cut);

        if (HasKey(node, "Cuts")) {
            const auto cuts = ReadRequiredKey<std::vector<double>>(node, "Cuts");
            if (cuts.size() != 3) { throw std::runtime_error("DSCuts YAML error: 'Cuts' must contain exactly 3 values: {mean, lower, upper}."); }
            mean = cuts.at(0);
            lower_cut = cuts.at(1);
            upper_cut = cuts.at(2);
        ...
        if (metadata_changed) {
            DSCuts rebuilt(cut_variable, region, particle, applied_cuts, mean, lower_cut, upper_cut);
            rebuilt.MeanFromHistogram = s.MeanFromHistogram;
            rebuilt.MeanFromFit = s.MeanFromFit;
            rebuilt.FitStdFactor = s.FitStdFactor;
            rebuilt.FitStd = s.FitStd;
            rebuilt.SetSliceNumber(s.GetSliceNumber());
            rebuilt.SetSliceUpperb(s.GetSliceUpperb());
            rebuilt.SetSliceLowerb(s.GetSliceLowerb());
            s = rebuilt;
        }

        // Public fit state and slice metadata can then be overridden directly.
        TryReadKey(node, "MeanFromHistogram", s.MeanFromHistogram);
        TryReadKey(node, "MeanFromFit", s.MeanFromFit);
        TryReadKey(node, "FitStdFactor", s.FitStdFactor);
        TryReadKey(node, "FitStd", s.FitStd);

        int slice_number = s.GetSliceNumber();
        double slice_upper = s.GetSliceUpperb();
        double slice_lower = s.GetSliceLowerb();

        if (TryReadKey(node, "SliceNumber", slice_number)) { s.SetSliceNumber(slice_number); }
        if (TryReadKey(node, "SliceUpperb", slice_upper)) { s.SetSliceUpperb(slice_upper); }
        if (TryReadKey(node, "SliceLowerb", slice_lower)) { s.SetSliceLowerb(slice_lower); }
    }

    static void Validate(const DSCuts& s) {
        if (s.Cuts.size() != 3) { throw std::runtime_error("DSCuts validation error: Cuts must contain exactly 3 values."); }

        if (s.GetLowerCut() > s.GetUpperCut()) { throw std::runtime_error("DSCuts validation error: LowerCut cannot be greater than UpperCut."); }
    }
};
