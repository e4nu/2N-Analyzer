...
class TargetParameters {
   ...
   public:
    ...
    std::string GetTargetElement() const { return TargetElement; }

    // Return the target PDG ion code used by GENIE.
    int GetTargetElementPDG() const { return TargetElementPDG; }

    // Return the probe PDG code.
    int GetProbePDG() const { return ProbePDG; }

    // Return the configured binding energy.
    double GetBindingEnergyToNucleus() const { return BindingEnergyToNucleus; }
};
