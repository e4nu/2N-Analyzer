...
// Include setup (old):
#include "../TwoNAnalyzer/setup/codeSetup.h"

// Header file for the classes stored in the TTree if any.

/**
 * @class gst
 * @brief A class to analyze the GENIE Standard Tree (GST) data, including member variables for the tree structure, leaf types, and branches.
 * @details This class is designed to analyze the GENIE Standard Tree (GST) data, which contains information about neutrino interactions. The class includes member variables for the TTree
 * structure, leaf types corresponding to the variables stored in the tree, and branches for accessing the data. The class can be used to read and analyze the GST data, allowing for the
 * extraction of relevant information about neutrino interactions for further analysis in the context of the 2N-Analyzer-e4nu project. It is important to ensure that the GST data is properly
 * formatted and that the TTree structure is correctly defined when using this class for analysis.
 * @note The class relies on the ROOT framework for handling the TTree structure and accessing the data stored in the GST. It is important to ensure that the ROOT environment is properly set
 * up when using this class for analysis. Additionally, users should be familiar with the structure of the GST data and the variables stored in the tree to effectively utilize this class for
 * extracting relevant information about neutrino interactions. The class can be extended or modified as needed to accommodate specific analysis requirements or to include additional
 * functionality for analyzing the GST data.
 */
class gst {
   public:
...
gst::gst(TTree* tree) : fChain(0) {
    ...
    if (tree == 0) {
        ...
        if (file_name == "general_file") {  // general_file (asportes_11_1000060120_2222_wfsi_SuSAv2_fixed_seed_555_test_17.gst.root)
            ...
            if (!f || !f->IsOpen()) { f = new TFile(InputHipoFiles.c_str()); }
        }
