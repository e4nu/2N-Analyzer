...
void Debugger::SafetyCheck_clas12ana_particles(const char *FILE, const int LINE, const bool &clas12ana_particles, std::vector<region_part_ptr> &allParticles, const int Nf) {
    if ((clas12ana_particles) && (allParticles.size() != Nf)) { PrintErrorMessage(FILE, LINE, "allParticles.size() is different than Nf!", ""); }
}
...
void Debugger::SafetyCheck_Reco_leading_FD_neutron(const char *FILE, const int LINE, const bool &apply_nucleon_cuts, const bool &ES_by_leading_FDneutron, const int &NeutronsFD_ind_mom_max,
    ...
    if (ES_by_leading_FDneutron) {
        ...
        if (NeutronsFD_ind_mom_max != -1) {
            ...
            if (!(LeadingnFDECIN || LeadingnFDECOUT)) { PrintErrorMessage(FILE, LINE, "Leading reco nFD check: no neutron hit in ECIN or ECOUT!", ""); }
        }

        // Safety check for leading nFD assignment
        if ((NeutronsFD_ind.size() > 0) && (NeutronsFD_ind_mom_max == -1)) { PrintErrorMessage(FILE, LINE, "Leading reco nFD check: leading was not assigned!", ""); }

        if (NeutronsFD_ind.size() == 1) {
            if (NeutronsFD_ind.at(0) != NeutronsFD_ind_mom_max) { PrintErrorMessage(FILE, LINE, "Leading reco nFD check: leading was assigned incorrectly!", ""); }
            ...
            for (int &i : NeutronsFD_ind) {
                ...
                if (dMomentum < 0) { PrintErrorMessage(FILE, LINE, "Leading reco nFD check: assigned nFD is not the leading!", ""); }
            }
        }
    }
}
...
void Debugger::SafetyCheck_Truth_leading_FD_neutron(const char *FILE, const int LINE, const bool &ES_by_leading_FDneutron, const int &TL_IDed_Leading_nFD_ind,
    ...
    if (ES_by_leading_FDneutron) {
        ...
        if ((TL_NeutronsFD_mom_ind.size() > 0) && (TL_IDed_Leading_nFD_ind == -1)) { PrintErrorMessage(FILE, LINE, "Leading TL nFD check: leading was not assigned!", ""); }

        if (TL_NeutronsFD_mom_ind.size() == 1) {
            if (TL_NeutronsFD_mom_ind.at(0) != TL_IDed_Leading_nFD_ind) { PrintErrorMessage(FILE, LINE, "Leading TL nFD check: leading was assigned incorrectly!", ""); }
...
void Debugger::SafetyCheck_FD_neutron(const char *FILE, const int LINE, const bool &apply_nucleon_cuts, std::vector<region_part_ptr> &allParticles, const DSCuts &n_mom_th,
    ...
    for (int i = 0; i < NeutronsFD_ind.size(); i++) {
        ...
        for (int j = i + 1; j < NeutronsFD_ind.size(); j++) {
            if (NeutronsFD_ind.at(i) == NeutronsFD_ind.at(j)) { PrintErrorMessage(FILE, LINE, "FD neutron check: duplicated FD neutrons!", ""); }
        }
    }
}
...
void Debugger::SafetyCheck_AMaps_Reco_leading_neutrons(const char *FILE, const int LINE, std::vector<region_part_ptr> &allParticles, const int &NeutronsFD_ind_mom_max,
    ...
    if (!(hitECIN_1e_cut || hitECOUT_1e_cut)) { PrintErrorMessage(FILE, LINE, "Leading reco nFD check (AMaps & WMaps): no neutron hit in ECIN or ECOUT!", ""); }
}
...
void Debugger::SafetyCheck_one_good_electron(const char *FILE, const int LINE, std::vector<region_part_ptr> &electrons) {
    if (electrons.size() != 1) { PrintErrorMessage(FILE, LINE, "1e cut: single electron cut is not implemented!", ""); }
}
...
void Debugger::SafetyCheck_1e_cut_electron(const char *FILE, const int LINE, std::vector<region_part_ptr> &electrons, const vector<int> &Electron_ind) {
    ...
    if (electrons[0]->getRegion() != FD) { PrintErrorMessage(FILE, LINE, "1e cut: electrons is not in the FD!", ""); }
}
...
void Debugger::SafetyCheck_basic_event_selection(const char *FILE, const int LINE, const std::string FinaleState, std::vector<region_part_ptr> &Kplus, std::vector<region_part_ptr> &Kminus,
    ...
    if (Piplus_ind.size() != 0) { PrintErrorMessage(FILE, LINE, (FinaleState + ": Piplus_ind.size() is different than 0!"), ""); }

    if (Piminus_ind.size() != 0) { PrintErrorMessage(FILE, LINE, (FinaleState + ": Piminus_ind.size() is different than 0!"), ""); }

    if (Electron_ind.size() != 1) { PrintErrorMessage(FILE, LINE, (FinaleState + ": Electron_ind.size() is different than 1!"), ""); }

    if (deuterons.size() != 0) { PrintErrorMessage(FILE, LINE, (FinaleState + ": deuterons.size() is different than 0!"), ""); }
}
...
void Debugger::SafetyCheck_1n(const char *FILE, const int LINE, vector<int> &NeutronsFD_ind, region_part_ptr &e_1n, region_part_ptr &n_1n, const bool &Enable_FD_photons,
    ...
    if (ES_by_leading_FDneutron) {  // TODO: add this check to nFDpCD
        ...
        for (int &i : NeutronsFD_ind) {
            if (i != NeutronsFD_ind_mom_max) {
                ...
                if (P_max_test < P_temp) { PrintErrorMessage(FILE, LINE, "1n: NeutronsFD_ind_mom_max is not leading FD neutron!", ""); }
            }
        }
    }
    ...
    if (!(NeutronInECIN_1n || NeutronInECOUT_1n)) { PrintErrorMessage(FILE, LINE, "1n: no neutron hit in ECIN or ECOUT!", ""); }
