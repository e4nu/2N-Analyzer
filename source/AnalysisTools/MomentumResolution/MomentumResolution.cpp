...
namespace {
[[noreturn]] void ExitMomentumResolutionError(const char* functionName, const char* fileName, const int lineNumber, const std::string& message) {
    bt::ExitWithError(functionName, fileName, lineNumber, message);
}
}  // namespace

#define MOMRES_EXIT(message_expr)                                                                \
    do {                                                                                         \
        std::ostringstream momentumResErrorStream;                                               \
        momentumResErrorStream << message_expr;                                                  \
        ExitMomentumResolutionError(__func__, __FILE__, __LINE__, momentumResErrorStream.str()); \
    } while (false)
...
MomentumResolution::MomentumResolution(const std::string& Particle) {
    ...
    if (Particle == "Neutron") {
        ...
        MOMRES_EXIT("entered particle is illegal!");
    }

#pragma region /* Safety checks */
    if (isNeutron == isProton) { MOMRES_EXIT("unclear nucleon selection!"); }

    if (MomResParticle != "Neutron" && MomResParticle != "Proton") { MOMRES_EXIT("nucleon have not been set!\nParticle = " << Particle << "\nMomResParticle = " << MomResParticle); }
#pragma endregion
}
#pragma endregion
...
void MomentumResolution::MomResInit(const MomentumResolutionSettings& MomResSettings, const std::string& SampleName, const double& beamE, const DSCuts& MomRes_mu_cuts,
                                    const DSCuts& MomRes_sigma_cuts, const double& ParticleMomTh, const std::string& SavePath, const bool& FitAggregatedHistograms, const bool& FitDebugging,
                                    const std::string& NucleonCutsDirectory, const std::string& MomentumResolutionDirectory) {
    const bool plot_and_fit_MomRes = MomResSettings.plot_and_fit_MomRes;
    const bool VaryingDelta = MomResSettings.VaryingDelta;
    const bool Calculate_momResS2 = MomResSettings.Calculate_momResS2;
    const bool Run_with_momResS2 = MomResSettings.Run_with_momResS2;
    const double DeltaSlices = MomResSettings.DeltaSlices;
    const bool ForceSmallpResLimits = MomResSettings.ForceSmallpResLimits;
    this->FitAggregatedHistograms = MomResSettings.FitAggregatedHistograms;
    AggregationEventTh = MomResSettings.AggregationEventTh;
    const std::string& SmearM = MomResSettings.SmearMode;
    const std::string& CorrM = MomResSettings.CorrMode;
    const bool momRes_test = MomResSettings.momRes_test;

    if (isNeutron) {
        bt::PrintMessage("[MomentumResolution::MomResInit (neutron)] Starting...", false);

        if (plot_and_fit_MomRes) {
            bt::PrintMessage("[MomentumResolution::MomResInit (neutron)] Initiating calculation mode...", false);
            ...
            bt::PrintMessage("[MomentumResolution::MomResInit (neutron)] Done.", false);

            bt::PrintMessage("[MomentumResolution::MomResInit (neutron)] Running in testing configuration.", false);
            ...
            bt::PrintMessage("[MomentumResolution::MomResInit (neutron)] Initiating loading mode...", false);
...
#pragma region /* Safety checks for data files */
                if (!bt::FindSubstring(NeutronCorrectionDataFile, "Neutron") || bt::FindSubstring(NeutronCorrectionDataFile, "Proton")) {
                    MOMRES_EXIT("neutron correction variables are not being loaded from neutron data!");
                }

                if (!bt::FindSubstring(ProtonSmearingDataFile, "Neutron") || bt::FindSubstring(ProtonSmearingDataFile, "Proton")) {
                    MOMRES_EXIT("proton smearing variables are not being loaded from neutron data!");
                }
#pragma endregion
            } else {  // if Calculate_momResS2 = false and Run_with_momResS2 = false => load both correction and smearing from momResS1
                std::string NeutronCorrectionDataFile = MomentumResolutionDirectory + "Res_data_-_" + SampleName + "/Neutron_momResS1_fit_param_-_" + SampleName + ".par";
...
#pragma endregion
            ...
            bt::PrintMessage("[MomentumResolution::MomResInit (neutron)] Done.");
        }
    } else if (isProton) {
        bt::PrintMessage("[MomentumResolution::MomResInit (proton)] Starting...", false);

        if (plot_and_fit_MomRes) {
            bt::PrintMessage("[MomentumResolution::MomResInit (proton)] Running in calculation mode. Setting up momentum slices...", false);
            ...
            bt::PrintMessage("[MomentumResolution::MomResInit (proton)] Done.");
        }
    }
}
#pragma endregion
...
#pragma region /* Safety check */
    if (momResS2CalcMode && momResS2RunMode) { MOMRES_EXIT("calculating and running on momResS2 is illegal!"); }
#pragma endregion
...
#pragma region /* Safety check */
        if ((abs(SliceUpperLim) > beamE * 1.5) || (abs(SliceLowerLim) > beamE * 1.5)) { MOMRES_EXIT("slice limits are not defined properly!"); }
#pragma endregion
                ...
                if (MomentumType == "truth") {
                    if (beamE == 5.98636) {
                        if (SampleName == "C12_simulation_G18_Q204_6GeV") {
                            ...
                            MOMRES_EXIT("no set of varying delta is available for '" << SampleName << "'!");
                        }
                    } else if (beamE == 2.07052) {
...
void MomentumResolution::SliceFitDrawAndSaveByType(const std::string& SampleName, const double& beamE) {
    if (!FitAggregatedHistograms) {
        ...
        SliceFitDrawAndSaveAggregated(SampleName, beamE, "truth", ResTLMomSlices, ResTLMomSlicesLimits, ResTLMomSlicesFitVar, ResTLMomSlicesHistVar, FittedTLMomSlices, TL_NumberOfSlices);
        SliceFitDrawAndSaveAggregated(SampleName, beamE, "reco", ResRecoMomSlices, ResRecoMomSlicesLimits, ResRecoMomSlicesFitVar, ResRecoMomSlicesHistVar, FittedRecoMomSlices,
                                      Reco_NumberOfSlices);
    }
}
#pragma endregion

#pragma region /* SliceFitDrawAndSaveAggregated function */
void MomentumResolution::SliceFitDrawAndSaveAggregated(const std::string& SampleName, const double& beamE, const std::string& MomentumType, std::vector<hPlot1D>& ResSlices0,
    ...
    if (NumberOfSlices0 <= 0) {
        SliceFitDrawAndSave(SampleName, beamE, MomentumType, ResSlices0, ResSlicesLimits0, ResSlicesFitVar0, ResSlicesHistVar0, FittedSlices0, NumberOfSlices0);
        return;
    }

    bt::PrintWarning(__func__, __FILE__, __LINE__, "Initiating slice fit, draw, and save with aggregated resolution slices...");

    auto BuildSliceMetadata = [&](hPlot1D& SourceSlice, DSCuts SourceFitCuts, DSCuts SourceHistCuts, const double SliceLowerLim, const double SliceUpperLim,
                                  const int SliceNumber) -> std::tuple<hPlot1D, DSCuts, DSCuts> {
        int SliceUpperLimPrecision = (SliceUpperLim == beamE) ? 3 : 2;
        ...
        hPlot1D AggregatedSlice(SourceSlice.GetFinalState(), SourceSlice.GetDetectorRegion(), hStatsTitle, hTitle, SourceSlice.GetXaxisTitle(), SourceSlice.GetHistogram1DSaveNamePath(),
                                hSaveName, SourceSlice.GetLowerXlim(), SourceSlice.GetUpperXlim(), SourceSlice.GetHistogramNumberOfXBins());
        TH1D* AggregatedHistogram = AggregatedSlice.GetHistogram();
        AggregatedHistogram->Sumw2();

        SourceFitCuts.SetCutVariable("fit_" + MomentumType + "_" + hCutName);
        SourceFitCuts.SetSliceNumber(SliceNumber);
        SourceFitCuts.SetSliceLowerb(SliceLowerLim);
        SourceFitCuts.SetSliceUpperb(SliceUpperLim);

        SourceHistCuts.SetCutVariable("hist_" + MomentumType + "_" + hCutName);
        SourceHistCuts.SetSliceNumber(SliceNumber);
        SourceHistCuts.SetSliceLowerb(SliceLowerLim);
        SourceHistCuts.SetSliceUpperb(SliceUpperLim);

        return {AggregatedSlice, SourceFitCuts, SourceHistCuts};
    };

    std::vector<hPlot1D> AggregatedSlices;
    std::vector<std::vector<double>> AggregatedSliceLimits;
    std::vector<DSCuts> AggregatedFitVars;
    std::vector<DSCuts> AggregatedHistVars;

    int StartSlice = 0;
    int AggregatedSliceNumber = 0;

    while (StartSlice < NumberOfSlices0) {
        int EndSlice = StartSlice;
        double AggregatedEvents = ResSlices0.at(StartSlice).GetHistogram()->Integral();

        while (AggregatedEvents < AggregationEventTh && EndSlice + 1 < NumberOfSlices0) {
            ++EndSlice;
            AggregatedEvents += ResSlices0.at(EndSlice).GetHistogram()->Integral();
        }

        const double SliceLowerLim = ResSlicesLimits0.at(StartSlice).at(0);
        const double SliceUpperLim = ResSlicesLimits0.at(EndSlice).at(1);

        std::tuple<hPlot1D, DSCuts, DSCuts> AggregatedSliceData =
            BuildSliceMetadata(ResSlices0.at(StartSlice), ResSlicesFitVar0.at(StartSlice), ResSlicesHistVar0.at(StartSlice), SliceLowerLim, SliceUpperLim, AggregatedSliceNumber);
        hPlot1D AggregatedSlice = std::get<0>(AggregatedSliceData);
        DSCuts AggregatedFitCuts = std::get<1>(AggregatedSliceData);
        DSCuts AggregatedHistCuts = std::get<2>(AggregatedSliceData);

        TH1D* AggregatedHistogram = AggregatedSlice.GetHistogram();
        for (int SliceIndex = StartSlice; SliceIndex <= EndSlice; SliceIndex++) { AggregatedHistogram->Add(ResSlices0.at(SliceIndex).GetHistogram()); }

        AggregatedSlices.push_back(AggregatedSlice);
        AggregatedSliceLimits.push_back({SliceLowerLim, SliceUpperLim});
        AggregatedFitVars.push_back(AggregatedFitCuts);
        AggregatedHistVars.push_back(AggregatedHistCuts);

        StartSlice = EndSlice + 1;
        AggregatedSliceNumber++;
    }

    ResSlices0 = AggregatedSlices;
    ResSlicesLimits0 = AggregatedSliceLimits;
    ResSlicesFitVar0 = AggregatedFitVars;
    ResSlicesHistVar0 = AggregatedHistVars;
    NumberOfSlices0 = static_cast<int>(ResSlices0.size());
...
#pragma region /* Setting polynomial */
    ...
    if (MomentumFitRange == "noKC") {
        ...
        MOMRES_EXIT("MomentumFitRange is illegal!");
    }
#pragma endregion
...
#pragma region /* Setting fitted slices */
    ...
    if (MomentumType == "truth") {
        ...
        MOMRES_EXIT("MomentumType is illegal!");
    }
#pragma endregion
...
#pragma region /* Setting variable to fit */
    ...
    if (FitType == "Smear") {
        ...
        MOMRES_EXIT("FitType is illegal!");
    }
#pragma endregion
...
#pragma region /* Safety check */
    if (MeanPn.size() != Pn_FitVar.size()) { MOMRES_EXIT("x and y data are of different lengths!"); }
#pragma endregion
...
#pragma region /* Safety checks for loading correction variables */
                    if (Loaded_Corr_coefficients_path == "") { MOMRES_EXIT("Loaded_Corr_coefficients_path is empty!\nThe file:\n" << filename << "\nis not found!"); }
#pragma endregion
...
#pragma region /* Safety checks for loading smearing variables */
                    if (Load_smearing && Loaded_Std_coefficients_path == "") { MOMRES_EXIT("Loaded_Std_coefficients_path is empty!\nThe file:\n" << filename << "\nis not found!"); }
#pragma endregion
...
#pragma region /* Safety checks */
                if (Loaded_Smear_coefficients_values.size() != Loaded_Smear_coefficients_names.size()) { MOMRES_EXIT("smearing coefficients load registered improperly!"); }

                if (Loaded_Corr_coefficients_values.size() != Loaded_Corr_coefficients_names.size()) { MOMRES_EXIT("correction coefficients load registered improperly!"); }
#pragma endregion
            }
        }
    } else {
        bt::PrintWarning(__func__, __FILE__, __LINE__, std::string("File not found! Target file was set to:\n") + filename);
        // bt::ExitWithError(__func__, __FILE__, __LINE__, "MomentumResolution::ReadResDataParam: file not found!");
    }
...
#pragma region /* Safety checks */
        if (!bt::FindSubstring(parameter, "error") && !bt::FindSubstring(parameter, "FitGoodness")) {
            if (Loaded_FitVarResults.size() != (PolynomialDegree + 1)) {
                ...
                details << "the parameter " << parameter << " loaded improperly!\n"
                        << "Loaded_FitVarResults.size() = " << Loaded_FitVarResults.size() << "\n"
                        << "expected size (= PolynomialDegree) = " << PolynomialDegree << "\n"
                        << "Loaded_FitVarResults elements:\n";
                for (int i = 0; i < Loaded_FitVarResults.size(); i++) { details << Loaded_FitVarResults.at(i) << "\n"; }
                MOMRES_EXIT(details.str());
            }
        }
        ...
        if (bt::FindSubstring(parameter, "error") && !bt::FindSubstring(parameter, "FitGoodness")) {
            if (Loaded_FitVarResultsErrors.size() != (PolynomialDegree + 1)) {
                std::ostringstream details;
                details << "the parameter " << parameter << " loaded improperly!\n"
                        << "Loaded_FitVarResultsErrors.size() = " << Loaded_FitVarResultsErrors.size() << "\n"
                        << "expected size (= PolynomialDegree) = " << PolynomialDegree << "\n"
                        << "Loaded_FitVarResultsErrors elements:\n";
                for (int i = 0; i < Loaded_FitVarResultsErrors.size(); i++) { details << Loaded_FitVarResultsErrors.at(i) << "\n"; }
                MOMRES_EXIT(details.str());
            }
        }
        ...
        if (!bt::FindSubstring(parameter, "error") && bt::FindSubstring(parameter, "FitGoodness")) {
            if (Loaded_FitVarResultsGoodness.size() != 2) {
                std::ostringstream details;
                details << "the parameter " << parameter << " loaded improperly!\n"
                        << "Loaded_FitVarResultsGoodness.size() = " << Loaded_FitVarResultsGoodness.size() << "\n"
                        << "expected size = 2\n"
                        << "Loaded_FitVarResultsGoodness elements:\n";
                for (int i = 0; i < Loaded_FitVarResultsGoodness.size(); i++) { details << Loaded_FitVarResultsGoodness.at(i) << "\n"; }
                MOMRES_EXIT(details.str());
            }
        }
#pragma endregion
                ...
                for (DSCuts Loaded_res_slice : Loaded_Res_Slices_FitVar) {
                    if ((Loaded_res_slice.GetSliceLowerb() < Momentum) && (Loaded_res_slice.GetSliceUpperb() >= Momentum)) {
                        if (RandomNumGenerator == "TRandom1") {
                            ...
                            MOMRES_EXIT("RandomNumGenerator illegal!");
                        }
...
#pragma endregion
            ...
            if (SmearMode == "pol1") {
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Smear_pol1.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol1 wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Smear_pol1.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol1 smearing wasn't loaded!"); }
#pragma endregion
...
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Smear_pol1_wKC.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol1_wKC wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Smear_pol1_wKC.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol1_wKC smearing wasn't loaded!"); }
#pragma endregion
...
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Smear_pol2.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol2 wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Smear_pol2.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol2 smearing wasn't loaded!"); }
#pragma endregion
...
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Smear_pol2_wKC.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol2_wKC wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Smear_pol2_wKC.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol2_wKC smearing wasn't loaded!"); }
#pragma endregion
...
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Smear_pol3.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol3 wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Smear_pol3.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol3 smearing wasn't loaded!"); }
#pragma endregion
...
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Smear_pol3_wKC.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol3_wKC wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Smear_pol3_wKC.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Smear_pol3_wKC smearing wasn't loaded!"); }
#pragma endregion
                ...
                MOMRES_EXIT("SmearMode is not a polynomial!");
            }
...
#pragma endregion
            ...
            if (CorrMode == "pol1") {
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Corr_pol1.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol1 wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Corr_pol1.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol1 correction wasn't loaded!"); }
#pragma endregion
...
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Corr_pol1_wKC.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol1_wKC wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Corr_pol1_wKC.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol1_wKC correction wasn't loaded!"); }
#pragma endregion
...
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Corr_pol2.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol2 wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Corr_pol2.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol2 correction wasn't loaded!"); }
#pragma endregion
...
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Corr_pol2_wKC.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol2_wKC wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Corr_pol2_wKC.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol2_wKC correction wasn't loaded!"); }
#pragma endregion
...
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Corr_pol3.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol3 wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Corr_pol3.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol3 correction wasn't loaded!"); }
#pragma endregion
...
#pragma region /* Safety checks */
                if (Loaded_Reco_FitParam_Corr_pol3_wKC.size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol3_wKC wasn't loaded!"); }
                if (Loaded_Reco_FitParam_Corr_pol3_wKC.at(0).size() == 0) { MOMRES_EXIT("Loaded_Reco_FitParam_Corr_pol3_wKC correction wasn't loaded!"); }
#pragma endregion
