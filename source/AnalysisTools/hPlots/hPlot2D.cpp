...
hPlot2D::hPlot2D(std::string h2DtReactions, std::string fState, std::string dRegion, std::string hst, std::string ht, std::string xat, std::string yat, std::string sPath, std::string sName,
    ...
    const std::string subtitle =
        histogram_functions::JoinHistogram2DContext(Histogram2DTitles["Histogram2DTitleReactions"], Histogram2DTitles["FinalState"], Histogram2DTitles["DetectorRegion"]);
...
hPlot2D::hPlot2D(std::string fState, std::string dRegion, std::string hst, std::string ht, std::string xat, std::string yat, std::string sPath, std::string sName, double LowerXlim,
    ...
    const std::string subtitle = histogram_functions::JoinHistogram2DContext("", Histogram2DTitles["FinalState"], Histogram2DTitles["DetectorRegion"]);
    const std::string statTitle = subtitle.empty() ? Histogram2DTitles["HistogramStatTitle"] : Histogram2DTitles["HistogramStatTitle"] + " (" + subtitle + ")";
    const std::string histTitle =
        Histogram2DTitles["HistogramTitle"] + (subtitle.empty() ? "" : " (" + subtitle + ")") + ";" + Histogram2DTitles["XaxisTitle"] + ";" + Histogram2DTitles["YaxisTitle"];

    Histogram2D = new TH2D(statTitle.c_str(), histTitle.c_str(), HistogramNumberOfXBins, HistogramXAxisLimits.at(0), HistogramXAxisLimits.at(1), HistogramNumberOfYBins,
                           HistogramYAxisLimits.at(0), HistogramYAxisLimits.at(1));
}
...
#pragma region /* histPlotter2D function (regular) */
void hPlot2D::histPlotter2D(const std::string& SampleName, TCanvas* HistogramCanvas, TH2D* Histogram2D, TList* Histogram_list, std::vector<TObject*>& HistoList,
    ...
    if (zLogScalePlot) {
        ...
        const std::string saveName = histogram_functions::Build2DScaleSaveNameInFolder(Histogram2DSaveNameDir, "z_Log_Scale_plots", sNameFlag, Histogram2DSaveName, "_zLogScale");
        HistogramCanvas->SaveAs(saveName.c_str());
        hf::FixPDFOrientation(saveName);
    ...
    if (ZLinearScalePlot) {
        ...
        const std::string saveName = histogram_functions::Build2DScaleSaveName(Histogram2DSaveNameDir, sNameFlag, Histogram2DSaveName, "_zLinearScale");
        HistogramCanvas->SaveAs(saveName.c_str());
        hf::FixPDFOrientation(saveName);
...
void hPlot2D::hDrawAndSave(const std::string& SampleName, TCanvas* h2DCanvas, TList* hList, std::vector<TObject*>& HistoList, bool showStats) {
...
void hPlot2D::histPlotter2D(const std::string& SampleName, TCanvas* HistogramCanvas, TH2D* Histogram2D, TList* Histogram_list, std::vector<TObject*>& HistoList,
    ...
    if (zLogScalePlot) {
        HistogramCanvas->SetLogz(1);
        const std::string saveName = histogram_functions::Build2DScaleSaveName(Histogram2DSaveNameDir, sNameFlag, Histogram2DSaveName, "_zLogScale");
        HistogramCanvas->SaveAs(saveName.c_str());
        hf::FixPDFOrientation(saveName);
    ...
    if (ZLinearScalePlot) {
        ...
        const std::string saveName = histogram_functions::Build2DScaleSaveNameInFolder(Histogram2DSaveNameDir, "z_Linear_Scale_plots", sNameFlag, Histogram2DSaveName, "_zLinearScale");
        HistogramCanvas->SaveAs(saveName.c_str());
        hf::FixPDFOrientation(saveName);
...
void hPlot2D::hDrawAndSave(const std::string& SampleName, TCanvas* h2DCanvas, TList* hList, std::vector<TObject*>& HistoList, TF1* Beta_function1, TF1* Beta_function2, TF1* Beta_function3,
...
void hPlot2D::hDrawAndSave(const std::string& SampleName, TCanvas* h2DCanvas, TList* hList, std::vector<TObject*>& HistoList, TF1* Beta_function1, std::string particle1, bool showStats,
                           bool plot_legend) {
...
void hPlot2D::hDrawAndSave(const std::string& SampleName, TCanvas* h2DCanvas, TList* hList, std::vector<TObject*>& HistoList, TF1* Beta_function1, std::string particle1, TF1* Beta_function2,
