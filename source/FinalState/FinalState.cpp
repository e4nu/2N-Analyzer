...
FinalState::FinalState(const ExperimentParameters& Experiment, const FinalStateConfiguration::EventVariant& eventVariant,
                       const FinalStateConfiguration::FinalStateSignature& finalStateSignature, const HistogramDefinitionSettings& hDefSettings,
                       const clas12::region_part_ptr& electron, const std::vector<clas12::region_part_ptr>& protons, const std::vector<int>& Electron_ind,
                       const std::vector<int>& Protons_ind, std::vector<clas12::region_part_ptr>& Kplus, std::vector<clas12::region_part_ptr>& Kminus,
                       const std::vector<int>& Piplus_ind, const std::vector<int>& Piminus_ind, std::vector<clas12::region_part_ptr>& deuterons, const TLorentzVector& beam_4v,
                       const double& Pvx, const double& Pvy, const double& Pvz, const CutSettings& CutSettings, const parameters_struct& parameters, nucleonResolution& nRes, Type type)
    : FinalStateHistograms(Experiment, eventVariant, finalStateSignature, hDefSettings) {
    this->type = type;
    ...
    if (p1->getRegion() == clas12::FD) {
        NucFD = p1;
        pCD = p2;
    } else if (p1->getRegion() == clas12::CD) {
        pCD = p1;
        NucFD = p2;
    } else {
...
void FinalState::SetAnalysisCuts(const CutSettings& CutSettings, const CutManager& CutManager, AMapsManager& aMaps_master, const AMapsSettings& AMapsSettings,
                                 const ESSettings& ESSettings) {
    FD_Theta_Cut = ((P_pFD_3v.Theta() * 180.0 / pi) <= CutManager.FD_nucleon_theta_cut.GetUpperCut());
    ...
    e_withinFC = aMaps_master.IsInFDQuery((!CutSettings.apply_fiducial_cuts || (AMapsSettings.Generate_Electron_AMaps || AMapsSettings.Generate_Nucleon_AMaps)), CutManager.ThetaFD,
                                          "Electron", P_e_3v.Mag(), P_e_3v.Theta() * 180.0 / pi, P_e_3v.Phi() * 180.0 / pi);
    ...
    pFD_withinFC = aMaps_master.IsInFDQuery((!CutSettings.apply_fiducial_cuts || (AMapsSettings.Generate_Electron_AMaps || AMapsSettings.Generate_Nucleon_AMaps)), CutManager.ThetaFD,
                                            "Proton", NucFD->getP(), P_pFD_3v.Theta() * 180.0 / pi, P_pFD_3v.Phi() * 180.0 / pi, ESSettings.Calc_eff_overlapping_FC);
...
void FinalState::runSafetyChecks(const std::vector<int>& Electron_ind, const clas12::region_part_ptr& electron, const clas12::region_part_ptr& p1, const clas12::region_part_ptr& p2,
    ...
    if (type == TwoNuc_pFDpCD) {
        if (electron->getRegion() != clas12::FD) { std::cout << "\033[33m\n\npFDpCD: Electron is not in the FD! Aborting...\n\n", exit(1); }
