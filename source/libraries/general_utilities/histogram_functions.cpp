...
void FillByInthsPlots(hsPlots& hsPlots_All_Int, hsPlots& hsPlots_QEL, hsPlots& hsPlots_MEC, hsPlots& hsPlots_RES, hsPlots& hsPlots_DIS, hsPlots::HistoType type, const bool& qel,
                      const bool& mec, const bool& res, const bool& dis, const double& Slice_variable, const double& Variable_x, const double& Variable_y, const double& Weight) {
    bool PrintOut = false;
 ...
 */
void ReassignPDFBookmarks(const std::string WorkingDir, const std::string& inputPDF, const std::string& outputPDF, bool hierarchical) {
...
void DrawAndSaveHistogramsToPDF(TCanvas* MainCanvas, const std::vector<TObject*>& HistoList, const std::string& Histogram_OutPDF_fileName_str, const std::string& SampleName,
    ...
    const std::string histogramOutPDFOpen = Histogram_OutPDF_fileName_str + "[";
    const std::string histogramOutPDFClose = Histogram_OutPDF_fileName_str + "]";

    TextCanvas->SaveAs(histogramOutPDFOpen.c_str());
    TextCanvas->cd();
    ...
    TextCanvas->Print(Histogram_OutPDF_fileName_str.c_str(), "pdf");
    TextCanvas->Clear();
    ...
    for (int i = 0; i < HistoList.size(); i++) {
        ...
        MainCanvas->Print(Histogram_OutPDF_fileName_str.c_str(), "pdf");
        MainCanvas->Clear();
    }

    MainCanvas->Print(histogramOutPDFClose.c_str(), "pdf");
}
#pragma endregion
 ...
 */
void DrawEmptyHistogramNotice(double x_1, double y_1, double x_2, double y_2, double diplayTextSize) {
